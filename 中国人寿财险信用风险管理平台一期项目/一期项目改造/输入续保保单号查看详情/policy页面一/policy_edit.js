...
var policyPolicyGrid = new Ext.grid.EditorGridPanel({
    title: "保单附加险信息管理",
    height: 200,
...
var collateralGrid = new Ext.grid.EditorGridPanel({
    title: "保单相关押品信息管理",
    height: 200,
    tbar: new Ext.Toolbar({
        autoWidth: true,
...
var guarantorGrid = new Ext.grid.EditorGridPanel({
    title: "保单相关担保人信息管理",
    height: 200,
    tbar: new Ext.Toolbar({
        autoWidth: true,
...
var onRenewal = function(m) {
	//var checkbox = new Ext.grid.CheckboxSelectionModel();
	var adSearch = "";
	var xbdhid = Ext.getCmp("policy.renewalCode").getValue();
       if (xbdhid == null || xbdhid == '') {
           Ext.MessageBox.alert("提示信息", "请输入续保保单号！");
       } else {
           ...
           var shenpiyijianStore = new Ext.data.Store({
               //autoDestroy: true,
               url: 'renewal_survey_list.action',
               autoLoad: true,
               baseParams: {
                   loadStore: true,
                   start: 0,
               ...
               reader: new Ext.data.JsonReader({
                   ...
                   {
                       name: 'policyStatus' // 保单状态
                   },
                   {
                       name: 'belongDeptName' //归属部门
                   },
                   {
                       name: 'salePersonName' //业务员/产险专员
                   },{
                	   name:'renewalCode'  //续保保单号
                   },{
                	   name:'id'
                   }
                   ]
...
var Panel_1 = new Ext.Panel({
    ...
    autoScroll: true,
    frame: true,
    html: "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=" + winWidth * 0.97 + " height=" + winHeight + " src='';>"
});
var Panel_2 = new Ext.Panel({
    ...
    autoScroll: true,
    id: 'C2',
    html: "<iframe frameborder='no' id='C2iframe' scrolling='false' border='0' marginwidth='0' marginheight='0' width=" + winWidth * 0.97 + " height=" + winHeight + " src='';>"
});
var Panel_3 = new Ext.Panel({
    ...
    autoScroll: true,
    frame: true,
    html: "<iframe frameborder='no' id='C3iframe' scrolling='false' border='0' marginwidth='0' marginheight='0' width=" + winWidth * 0.97 + " height=" + winHeight + " src='';>"
});
var Panel_4 = new Ext.Panel({
    ...
    autoScroll: true,
    frame: true,
    html: "<iframe frameborder='no' id='C4iframe' scrolling='false' border='0' marginwidth='0' marginheight='0' width=" + winWidth * 0.97 + " height=" + winHeight + " src='';>"
});
var Panel_5 = new Ext.Panel({
    ...
    autoScroll: true,
    frame: true,
    html: "<iframe frameborder='no'  id='C5iframe' scrolling='false' border='0' marginwidth='0' marginheight='0' width=" + winWidth * 0.97 + " height=" + winHeight + " src=''>"
});
var Panel_6 = new Ext.Panel({
    title: '资信评分',
    id: 'C6',
    frame: true,
    autoScroll: true,
    html: "<iframe frameborder='no'  id='C6iframe' scrolling='false' border='0' marginwidth='0' marginheight='0' width=" + winWidth * 0.97 + " height=" + winHeight + " src=''>"
});
var policyId=null;
var isFgshbg = null;
var recored=null;
var onCmdAS = function(m) {
	var grid2 = this.ownerCt.ownerCt;
   var obj = grid2.getSelectionModel().getSelections();	
   
    if (obj.length == 0) {
        Ext.MessageBox.alert("信息", "请选择要查看的的记录！");
        return;
    }
    recored = grid2.getSelectionModel().getSelected();
    	//alert(recored.data.id);
    	policyId=recored.data.id;
    //var policyId='8981f863504fd87201507e1e28260599'; 测试用
   
    if (Panel_1.html) { //第一次进入时，html属性有值，所有直接赋值更新路径
        //		   分公司核保岗
        if ('1' == isFgshbg) {
        	
            Panel_1.html = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "SPpolicy_view.action?id=" + policyId + "';>";
        } else {
            Panel_1.html = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "policyRenewal_view.action?id=" + policyId + "&isSurvey=1" + "';>";
        }
    } else { //第二次进入窗口时html会被update转换成body所已需要修改body中的iframe
        //		   分公司核保岗
        if ('1' == isFgshbg) {
        	
        	Panel_1.body.dom.innerHTML = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "SPpolicy_view.action?id=" + policyId + "';>";
        } else {
            Panel_1.body.dom.innerHTML = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "policyRenewal_view.action?id=" + policyId + "&isSurvey=1" + "';>";
        }
    }
    //	   	查询客户信息
    var customerAction = null;

    Ext.Ajax.request({
        url: 'getCustomerByPolicyId.action',
        params: {
            policyId: policyId
        },
        method: "GET",
        success: function(response, opts) {
            var customerdata = Ext.decode(response.responseText);
            if (customerdata.data == null) {
                Ext.MessageBox.alert("操作信息", "投保人客户不存在，请联系管理员!");
            }
            var customerType = customerdata.data.customerType;
            var customerId = customerdata.data.id;
            if (customerType == "2") {
                customerAction = "customerCompanyEdit_view.action?isSurvey=1&tag=1&id=" + customerId + '&isFgshbg=' + isFgshbg;
            } else {
                customerAction = "customerPersonEdit_view.action?isSurvey=1&tag=1&id=" + customerId + '&isFgshbg=' + isFgshbg;
            }	
            var accptanceWin = new Ext.Window({
                layout: 'card',
                title: '资信调查',
                modal: true,
                maximizable: true,
                maximized: true,
                activeItem: 0,
                width:500 ,//winWidth * 0.99
                bodyStyle: 'position:relative',
                closeAction: 'hide',
                height:300 ,//winHeight - 70
                tbar: [{
                    xtype: 'button',
                    id: 'applyInfo',
                    text: '核对申请详情',
                    ...
                    handler: function(b, e) {
                        clickPage(b, accptanceWin, insuranceType, customerAction);
                    }
                },
                {
                    xtype: 'tbtext',
                ...
                items: [Panel_1, Panel_2, Panel_3, Panel_4, Panel_5, Panel_6],
                 buttons: [/*{
            	   text: "查看影像",
                    recType: 1,
                    popup: true,
                ...
                },*/
                {
                    text: '上一步',
                    scope: this,
                    icon: "images/btn/top.gif",
                    hidden: true,
                    handler: function(b, e) {
                        //changePage(b, accptanceWin);
                    }
                },
                {
                    text: '下一步',
                    scope: this,
                    icon: "images/btn/down.gif",
                    handler: function(b, e) {
                        //changePage(b, accptanceWin);
                    }
                },
...
function clickPage(btn, win, insuranceType, customerAction) {
    if (insuranceType == '2205' && btn.id == "collerInfo") {
        Ext.MessageBox.alert("提示", "车贷险没有押品信息!");
        return;
    }
    var tbarIds = new Array('applyInfo', 'custInfo', 'creditInfo', 'collerInfo', 'surveyInfo', 'rateInfo');
    //var policyId = policyId;
    var t = btn;
    //清空所有的背景色
    for (var i = 0; i < tbarIds.length; i++) {
        var tb = Ext.getCmp(tbarIds[i]);
        tb.getEl().dom.style.backgroundColor = '';
    }
    //	       	当前背景色
    t.getEl().dom.style.backgroundColor = '#8F8FBD';

    if (btn.id == "applyInfo") {
        win.buttons[2].show();
        win.buttons[1].hide();
        win.doLayout();
        win.layout.setActiveItem(Panel_1);
    } else if (btn.id == "custInfo") {
        if (Panel_2.body.dom.innerHTML.indexOf('?') < 0 /*|| Panel_2.body.dom.innerHTML.indexOf(recored.data.policyHolder) < 0*/) {
            Panel_2.body.dom.innerHTML = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + customerAction + "';>";
        }
        win.doLayout();
        win.layout.setActiveItem(Panel_2);
        win.buttons[2].show();
        win.buttons[1].show();
    } else if (btn.id == "creditInfo") {
        if (Panel_3.body.dom.innerHTML.indexOf('?') < 0 || Panel_3.body.dom.innerHTML.indexOf(policyId) < 0) {
            Panel_3.body.dom.innerHTML = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "gather_credit.action?tag=1&id=" + policyId + "&isFgshbg=" + isFgshbg + "';>";
        }
        win.doLayout();
        win.layout.setActiveItem(Panel_3);
        win.buttons[2].show();
        win.buttons[1].show();
    } else if (btn.id == "collerInfo") {
        if (Panel_4.body.dom.innerHTML.indexOf('?') < 0 || Panel_4.body.dom.innerHTML.indexOf(policyId) < 0) {
            Panel_4.body.dom.innerHTML = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "importCollateral_view.action?tag=1&id=" + policyId + "&isFgshbg=" + isFgshbg + "';>";
        }
        win.doLayout();
        win.layout.setActiveItem(Panel_4);
        win.buttons[2].show();
        win.buttons[1].show();
    } else if (btn.id == "surveyInfo") {
        if (Panel_5.body.dom.innerHTML.indexOf('?') < 0 || Panel_5.body.dom.innerHTML.indexOf(policyId) < 0) {
            Panel_5.body.dom.innerHTML = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "questionaryRenewal_win.action?isReport=0&id=" + policyId + "&isFgshbg=" + isFgshbg + "';>";
        }
        win.doLayout();
        win.layout.setActiveItem(Panel_5);
        win.buttons[2].show();
        win.buttons[1].show();
    } else {
        win.buttons[2].hide();
        win.buttons[1].show();
        Panel_6.body.dom.innerHTML = "<iframe frameborder='no' scrolling='false' border='0' marginwidth='0' marginheight='0' width=1325.02 height=644 src='" + "getScore_view.action?tag=1&id=" + policyId + "&isFgshbg=" + isFgshbg + "';>";
        win.doLayout();
        win.layout.setActiveItem(Panel_6);
    }
}
//-------------------------------------------------------自定义修改结束
